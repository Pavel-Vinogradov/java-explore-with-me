...
public class StatsController {
    ...
    @GetMapping("/stats")
    public List<ResponseStatsDto> getStat(@RequestParam(required = false, name = "start") @DateTimeFormat(pattern = "yyyy-MM-dd HH:mm:ss") LocalDateTime start,
                                          @RequestParam(required = false, name = "end") @DateTimeFormat(pattern = "yyyy-MM-dd HH:mm:ss") LocalDateTime end,
                                          @RequestParam(name = "uris", required = false) ArrayList<String> uris,
                                          @RequestParam(value = "unique", defaultValue = "false") boolean unique,
                                          @RequestParam(value = "limit", defaultValue = "20") int limit) {
        if (start == null || end == null) {
            throw new ValidTimeException("Отсутствует дата старта или конца");
        }
